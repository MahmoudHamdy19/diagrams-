...
// Remove the existing class_diagram_erd diagram tab
const regex = /<diagram name="Class Diagram" id="class_diagram_erd">[\s\S]*?<\/diagram>/g;
mainFile = mainFile.replace(regex, "");

// Make sure it doesn't leave multiple </mxfile> tags if regex failed to catch something weird, but replace is straightforward
const updated = mainFile.replace("</mxfile>", xml);
fs.writeFileSync(filename, updated, "utf8");
console.log("Replaced Class Diagram tab with English version.");
