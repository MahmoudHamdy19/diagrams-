...
let isUtf16 = false;
// very naive check: if second byte is 0, assume utf-16 le
if (rawBuffer.length > 1 && rawBuffer[1] === 0) {
    isUtf16 = true;
}

let mainFile = fs.readFileSync(filename, isUtf16 ? "utf16le" : "utf8");

// Cleanup previously corrupted class_diagram if any
const regex = /<diagram name="Class Diagram" id="class_diagram_erd">[\s\S]*?<\/diagram>/g;
mainFile = mainFile.replace(regex, "");

// Append new diagram
let updated = mainFile.replace("</mxfile>", xml);

// Write back as UTF-8
fs.writeFileSync(filename, updated, "utf8");

console.log("Successfully converted file encoding and appended English class diagram.");
